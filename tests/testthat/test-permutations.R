...
test_that("properties of permutations are as expected", {
  ...
  permutations <- lime:::get_index_permutations(
    original_document,
    number_permutations
  )
  ...
  expect_true(all(unlist(lapply(permutations, function(x) {
    all(!duplicated(x))
  }))))
...
test_that("Cosine computation is correct", {
  ...
  expect_equal(
    lime:::cosine_distance_vector_to_matrix_rows(v, m),
    apply(m, MARGIN = 1, cos)
  )
})

test_that("there is no empty generated text", {
  generated_documents <- lime:::permute_cases.character(
  ...
  # There is no empty permutation
  expect_true(all(
    sapply(generated_documents$permutations, nchar, USE.NAMES = FALSE) > 0
  ))
})
