...
lime.data.frame <- function(
  ...
  explainer$x <- NULL
  explainer$feature_type <- setNames(
    sapply(x, function(f) {
      if (is.integer(f)) {
      ...
      if (explainer$feature_type[i] %in% c('numeric', 'integer')) {
        if (quantile_bins) {
          ...
          if (length(bins) < 3) {
            warning(
              names(x)[i],
              ' does not contain enough variance to use quantile binning. Using standard binning instead.',
              call. = FALSE
            )
  ...
  explainer$feature_distribution <- setNames(
    lapply(seq_along(x), function(i) {
      switch(
...
explain.data.frame <- function(
  ...
  res <- lapply(seq_along(case_ind), function(ind) {
    ...
    res$feature_desc <- describe_feature(
      res$feature,
      case_perm[i[1], ],
      explainer$feature_type,
      explainer$bin_continuous,
    ...
    res$label_prob <- unname(as.matrix(case_res[i[1], ]))[match(
      res$label,
      colnames(case_res)
    )]
