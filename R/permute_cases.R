...
permute_cases.data.frame <- function(
  ...
  perm <- as.data.frame(
    lapply(seq_along(cases), function(i) {
      perms <- if (
        is.numeric(cases[[i]]) &&
          identical(is.na(feature_distribution[[i]]), TRUE)
      ) {
        rep(cases[[i]], each = n_permutations)
      } else if (is.numeric(cases[[i]]) && bin_continuous) {
        bin <- sample(
          seq_along(feature_distribution[[i]]),
          nrows,
          TRUE,
        ...
        sample(
          feature_distribution[[i]]$x,
          nrows,
          TRUE,
          feature_distribution[[i]]$y
        ) +
          runif(nrows, -width, width)
      } else if (is.numeric(cases[[i]])) {
        rnorm(nrows) *
          feature_distribution[[i]]['sd'] +
          feature_distribution[[i]]['mean']
      } else if (is.character(cases[[i]])) {
        sample(
          names(feature_distribution[[i]]),
          nrows,
          TRUE,
          as.numeric(feature_distribution[[i]])
        )
        ...
        sample(
          as.logical(names(feature_distribution[[i]])),
          nrows,
          TRUE,
          as.numeric(feature_distribution[[i]])
        )
        ...
        inherits(cases[[i]], 'Date') || inherits(cases[[i]], 'POSIXt')
      ) {
        rep(cases[[i]], each = n_permutations)
      }
...
#' @exportS3Method
permute_cases.character <- function(
  cases,
  n_permutations,
  tokenization,
  keep_word_position,
    ...
    get_token_position <- function(tokenized_case, token_start_position) {
      if (keep_word_position) {
        tokens <- paste0(
          tokenized_case,
          "_",
          seq_along(tokenized_case) + token_start_position
        )
  ...
  word_selections <- lapply(documents_tokens, function(x) {
    get_index_permutations(x, n_permutations)
  })
  ...
  permutation_candidates <- sapply(
    word_selections_flatten,
    function(x) paste(tokens_for_external_model[x], collapse = " "),
    USE.NAMES = FALSE
  )
  ...
  permutation_distances <- local({
    compute_distances <- function(documents, indexes) {
      boolean_document_representation <- word_indexes_2_logical_vector(
        documents
      )
      start_index <- indexes[1]
      end_index <- indexes[2]
      cosine_distance_vector_to_matrix_rows(
        boolean_document_representation,
        bow_matrix[start_index:end_index, ]
      )
    }
    distances <- mapply(
      compute_distances,
