...
plot_features <- function(explanation, ncol = 2, cases = NULL) {
  ...
  if (explanation$model_type[1] == 'regression') {
    ...
    explanation[
      !binned_feature,
      "feature_weight"
    ] <- as.numeric(explanation$feature_value[!binned_feature]) *
      explanation$feature_weight[!binned_feature]
  }

  explanation$type <- factor(
    ifelse(sign(explanation$feature_weight) == 1, type_pal[1], type_pal[2]),
    levels = type_pal
  )
  description <- paste0(explanation$case, '_', explanation[['label']])
  desc_width <- max(nchar(description)) + 1
  description <- paste0(
    format(description, width = desc_width),
    explanation$feature_desc
  )
  explanation$description <- factor(
    description,
    levels = description[order(abs(explanation$feature_weight))]
  )
  ...
  if (explanation$model_type[1] == 'classification') {
    ...
    explanation$label <- factor(
      explanation$label,
      unique(explanation$label[order(
        explanation$label_prob,
        decreasing = TRUE
      )])
    )
    p <- ggplot(explanation) +
      facet_wrap(
        ~ case + label + probability + `Explanation fit`,
    ...
    p <- ggplot(explanation) +
      facet_wrap(
        ~ case + prediction + `Explanation fit`,
        labeller = label_both_upper,
        scales = 'free_y',
        ncol = ncol
      )
  ...
  p +
    ...
    scale_x_discrete(labels = function(lab) {
      substr(lab, desc_width + 1, nchar(lab))
    }) +
...
plot_explanations <- function(explanation, ...) {
  ...
  if (!anyNA(num_cases)) {
    explanation$case <- factor(
      explanation$case,
      levels = as.character(sort(num_cases))
    )
  }
  explanation$feature_desc <- factor(
    explanation$feature_desc,
    levels = rev(unique(explanation$feature_desc[order(
      explanation$feature,
